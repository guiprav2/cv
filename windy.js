...
function parseMods(mods) {
  let ret = { bp: null, sts: [] };
  for (let m of mods.split(':')) {
    let bpEntry = Object.entries(parseMods.bps).find(x => x[0] === m);
    ret.bp ||= bpEntry?.[1]; ret.bpid ||= bpEntry?.[0];
    let st = Object.entries(parseMods.sts).find(x => x[0] === m)?.[1];
    st && ret.sts.push([m, st]);
  }
  return ret;
}

parseMods.bps = { sm: '640px', md: '768px', lg: '1024px', xl: '1280px', '2xl': '1536px', print: 'print' };

parseMods.sts = {
  'group-hover': 'hover', hover: 'hover', active: 'active', visited: 'visited', target: 'target',
  empty: 'empty', disabled: 'disabled', enabled: 'enabled', checked: 'checked', required: 'required',
  placeholder: 'placeholder', focus: 'focus', 'focus-within': 'focus-within',
  'focus-visible': 'focus-visible', tmp: 'tmp', 'not-first': 'nth-child(n + 2)', first: 'first-child',
  last: 'last-child', only: 'only-child', after: ':after', before: ':before', children: '> *',
  divide: '> * + *', dark: 'dark', 'auto-dark': 'auto-dark',
};

function buildSelector(spec, semantic) {
  spec = spec.split(':');
  let name = spec.pop(), mods = spec.join(':');
  let { bp, bpid, sts } = parseMods(mods);
  let stsBefore = sts.map(x => x[0]);
  let stsAfter = sts.map(x => x[1]).filter(x => x !== 'tmp');
  let media = [];
  let parentSel = [];
  if (bp && bp !== 'print') {
    media.push(`(min-width: ${bp})`);
  }
  if (stsBefore.includes('auto-dark')) {
    media.push('(prefers-color-scheme: dark)');
    stsAfter.splice(stsAfter.indexOf('auto-dark'), 1);
  }
  if (stsBefore.includes('dark')) {
    parentSel.push('.dark');
    stsAfter.splice(stsAfter.indexOf('dark'), 1);
  }
  if (stsBefore.includes('group-hover')) {
    parentSel.push('.group:hover');
    stsAfter.splice(stsAfter.indexOf('hover'), 1);
  }
  let x = [
    bp === 'print' ? `@media print {` : media.length && `@media ${media.join(' and ')} {`,
      (parentSel.length ? `${parentSel.join(' ')} .` : '.') + CSS.escape(
        (!semantic && bp ? `${bpid}:` : '') +
        (!semantic && stsBefore.length ? `${stsBefore.join(':')}:` : '') +
        name
      ).replace(/\\\./g, '.') +
      (stsAfter.length ? `:${stsAfter.join(':')}`.replace(':>', ' >') : ''),
  ].filter(Boolean).join('\n');
  return x;
}

function buildStyles(xs) {
  let props = {};
  xs: for (let x of xs.split(/\s+/)) {
    if (x.startsWith('--')) {
      let [k, v] = x.split('=');
      props[k] = v;
      continue;
...
function buildRule(sel, styles, semantic) {
  let sel2 = buildSelector(sel, semantic);
  return [
    `${sel2} {`,
      ...buildStyles(styles).map(x => `  ${x}`.replace('};', '}')),
    '}',
    sel2.startsWith('@media') && '}',
  ].filter(Boolean).join('\n');
}

function buildRuleSet(sel, styles) {
  let semantic = sel !== styles, sets = {};
  for (let x of styles.split(/\s+/)) {
    let parts = x.split(':'), y = parts.pop(), mods = parts.join(':') || 'base';
    sets[mods] ||= '';
    sets[mods] = (sets[mods] + ` ${y}`).trim();
  }
  return Object.keys(sets).map(x => {
    let sel2 = semantic && x !== 'base' ? `${x}:${sel}` : sel;
    return buildRule(sel2, sets[x], semantic);
  }).join('\n\n');
}

window.windy = function windy(...args) {
  let name = arguments.length > 1 && args.shift();
  ...
  if (name) {
    ...
    for (let x of styles.split(/\s+/).filter(Boolean)) {
...
//title.classList.add(windy('MyBlock-myEl', 'rounded-xl shadow-md border p-5 border-red-200 bg-green-500 text-yellow-300'));

//windy('EditorShell', 'grid grid-cols-[280px_1fr] grid-rows-[auto_1fr] h-screen overflow-hidden rounded bg-gray-900');
//windy('EditorShellTopBar', 'col-span-2 flex justify-between overflow-hidden border-b-1 border-gray-800 text-white bg-gray-700');
//windy('EditorShell-leftPanel', 'flex flex-col overflow-auto border-r-1 border-gray-700 border-opacity-10 bg-gray-900 relative');
//windy('EditorShell-frame', 'w-full h-full bg-white');
//windy('EditorShellTopBar-iconBtn', 'flex rounded-sm p-2 text-inherit hover:text-gray-300 bg-transparent hover:bg-gray-600 cursor-pointer');
//windy('EditorShellTopBar-leftBox', 'flex-shrink-0 flex gap-1 p-2 text-gray-400');
//windy('EditorShellTopBar-rightBox', 'flex flex-basis-0 gap-2 items-center max-w-[60vw] px-2 text-gray-200');
//windy('EditorShellTopBar-draggableBtn', 'rounded-full px-3 py-0.5 text-sm text-gray-200 bg-gray-500 hover:bg-gray-400 hover:shadow-md transition-colors cursor-move');
